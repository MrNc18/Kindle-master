...
import { addBookCategoryService } from "../../services/book";
...
export default ({ handleClose }) => {
  ...
  const addCategory = async () => {
    try {
      setDisableBtn(true);
      const resp = await addBookCategoryService(name.trim(), desc.trim());
        ...
        <div>
          ...
          <Grid>
            <Button
              ...
              disabled={
                disableBtn || !isInputValid(name) || !isInputValid(desc)
              }
            >
              Add Category
            </Button>
